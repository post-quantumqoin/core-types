...
func (t *State) MarshalCBOR(w io.Writer) error {
	...
	// t.Cert ([]uint8) (slice)
	if len(t.Cert) > 2097152 {
		return xerrors.Errorf("Byte array in field t.Initcode was too long")
	}

	if err := cw.WriteMajorTypeHeader(cbg.MajByteString, uint64(len(t.Cert))); err != nil {
		return err
	}

	if _, err := cw.Write(t.Cert); err != nil {
		return err
	}
	return nil
}
...
func (t *State) UnmarshalCBOR(r io.Reader) (err error) {
	*t = State{}
	...
	if extra > 2097152 {
		return fmt.Errorf("t.Cert: byte array too large (%d)", extra)
	}
	...
	if extra > 0 {
		t.Cert = make([]uint8, extra)
	}

	if _, err := io.ReadFull(cr, t.Cert); err != nil {
		return err
	}
	return nil
}
