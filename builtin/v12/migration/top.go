...
import (
	...
	system11 "github.com/post-quantumqoin/core-types/builtin/v11/system"
	adt11 "github.com/post-quantumqoin/core-types/builtin/v11/util/adt"
	"github.com/post-quantumqoin/core-types/manifest"
	"github.com/post-quantumqoin/core-types/migration"
)
...
func MigrateStateTree(ctx context.Context, store cbor.IpldStore, newManifestCID cid.Cid, actorsRootIn cid.Cid, priorEpoch abi.ChainEpoch, cfg migration.Config, log migration.Logger, cache migration.MigrationCache) (cid.Cid, error) {
	...
	mm, err := newMinerMigrator(ctx, store, miner12Cid)
	if err != nil {
		return cid.Undef, xerrors.Errorf("failed to create miner migrator: %w", err)
	}
	migrations[miner11Cid] = migration.CachedMigration(cache, *mm)
