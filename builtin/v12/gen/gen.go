...
import (
	...
	"github.com/post-quantumqoin/core-types/builtin/v12/account"
	"github.com/post-quantumqoin/core-types/builtin/v12/cron"
	"github.com/post-quantumqoin/core-types/builtin/v12/datacap"
	"github.com/post-quantumqoin/core-types/builtin/v12/eam"
	"github.com/post-quantumqoin/core-types/builtin/v12/evm"
	init_ "github.com/post-quantumqoin/core-types/builtin/v12/init"
	"github.com/post-quantumqoin/core-types/builtin/v12/market"
	"github.com/post-quantumqoin/core-types/builtin/v12/miner"
	"github.com/post-quantumqoin/core-types/builtin/v12/multisig"
	"github.com/post-quantumqoin/core-types/builtin/v12/paych"
	"github.com/post-quantumqoin/core-types/builtin/v12/power"
	"github.com/post-quantumqoin/core-types/builtin/v12/reward"
	"github.com/post-quantumqoin/core-types/builtin/v12/system"
	"github.com/post-quantumqoin/core-types/builtin/v12/util/smoothing"
	"github.com/post-quantumqoin/core-types/builtin/v12/verifreg"
)

func main() {
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/system/cbor_gen.go", "system",
		// actor state
		system.State{},
	); err != nil {
		panic(err)
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/account/cbor_gen.go", "account",
		// actor state
		account.State{},
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/cron/cbor_gen.go", "cron",
		// actor state
		cron.State{},
		cron.Entry{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/reward/cbor_gen.go", "reward",
		// actor state
		reward.State{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/multisig/cbor_gen.go", "multisig",
		// actor state
		multisig.State{},
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/paych/cbor_gen.go", "paych",
		// actor state
		paych.State{},
		paych.LaneState{},
		//method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/power/cbor_gen.go", "power",
		// actors state
		power.State{},
		power.Claim{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/market/cbor_gen.go", "market",
		// actor state
		market.State{},
		market.DealState{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/miner/cbor_gen.go", "miner",
		// actor state
		miner.State{},
		miner.MinerInfo{},
		miner.Deadlines{},
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/verifreg/cbor_gen.go", "verifreg",
		// actor state
		verifreg.State{},
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/datacap/cbor_gen.go", "datacap",
		// actor state
		datacap.State{},
		datacap.TokenState{},
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/util/smoothing/cbor_gen.go", "smoothing",
		smoothing.FilterEstimate{},
	); err != nil {
		panic(err)
	}

	if err := gen.WriteTupleEncodersToFile("./builtin/v12/init/cbor_gen.go", "init",
		// actor state
		init_.State{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/evm/cbor_gen.go", "evm",
		// actor state
		evm.Tombstone{},
		evm.State{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v12/eam/cbor_gen.go", "eam",
		// method params and returns
		eam.CreateParams{},
		eam.CreateReturn{},
		eam.Create2Params{},
