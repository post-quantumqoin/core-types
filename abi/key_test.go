...
func newContractAddr(t testing.TB, data string) address.Address {
	address, err := address.NewContractAddress([]byte(data))
	if err != nil {
		t.Fatal(err)
	}
	return address
...
func TestAddrKey(t *testing.T) {
	...
	actor_address_1 := newContractAddr(t, "actor1")
	actor_address_2 := newContractAddr(t, "222")
